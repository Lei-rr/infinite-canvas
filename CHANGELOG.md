...
+ [调整] 上游不再提供普通 gpt-image-2，生图只保留 3 个 Gemini 对话模型和 4 个 l0veyou 模型。
+ [调整] 内置 BFF 重构为模块化结构（config / lib / channels / routes），渠道以统一接口注册，新增渠道不需要再改动前端。
+ [新增] 模型统一以 `渠道/模型` 命名，接口返回 `capability` 能力标签，前端启动时自动同步模型列表。
+ [新增] 前端默认渠道指向内置 BFF，并停用 API 配置入口（原实现以注释保留）。
+ [新增] l0veyou 渠道：网页登录态异步生图，登录态自动刷新并持久化到 `data/auth/l0veyou.json`。
+ [修复] 生图结果下载与上游回源增加内容类型校验，避免把上游 HTML 错误页当作图片写入本地。
+ [修复] 本地媒体补充 jpg / webp / mp4 等常见扩展名的 MIME 映射。
